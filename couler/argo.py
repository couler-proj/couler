...
def create_secret(secret_data, namespace="default", name=None, dry_run=False):
    ...
    # Avoid create the secret for same secret data
    if secret.name not in states._secrets and not dry_run:
        states._secrets[secret.name] = secret

    # Append secret key reference in env when `dry_run=True`
    if states.get_secret_envs(secret.name) is not None and dry_run:
        states._secret_envs[secret.name] = secret.to_env_list()
